...
export interface AgentState {
  ...
  respondPlan: (approved: boolean, feedback: string) => void
}
...
export function useAgentSocket(opts: UseAgentSocketOptions = {}): AgentState {
  ...
  // When the agent proposes a plan (present_plan), this holds the plan text and
  // the turn blocks server-side until the user approves or requests changes.
  ...
  useEffect(() => {
    ...
    socket.on('plan_proposed', (d: { plan: string }) => {
      closeStreaming()
      setPlanText(d.plan)
    })
    ...
    socket.on('turn_done', (d: { usage: UsageSummary }) => {
      ...
      closeStreaming()
      // A turn that ends (incl. error/stop) must not leave a stale plan card or a
      // stuck danger-confirm — the latter silently wedges the composer.
      setPlanText(null)
      setDangerPending(false)
      if (d?.usage) setUsage(d.usage)
      refresh()
  ...
  const setPlanMode = useCallback((on: boolean) => {
    socketRef.current?.emit('set_plan_mode', { on })
  }, [])

  const respondPlan = useCallback((approved: boolean, feedback: string) => {
    socketRef.current?.emit('respond_plan', { approved, feedback })
    setPlanText(null)
  }, [])
